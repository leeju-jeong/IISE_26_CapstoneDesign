...
def process_session(video_path: str, output_dir: str, cfg: dict,
                    model_path: str = DEFAULT_MODEL, flat: bool = False) -> None:
...
def main():
    ...
    if video_path.is_dir():
        for mp4 in sorted(video_path.glob("**/*.mp4")):
            process_session(str(mp4), args.output_dir, cfg, args.model, flat=flat)
    else:
        process_session(str(video_path), args.output_dir, cfg, args.model, flat=flat)
