...
실행:
  python tools/label_actions.py --video data/raw/subject_01.mp4 --output data/raw/subject_01_labels.csv
...
class Labeler:
    ...
    def press(self, action: int, t: float) -> str | None:
        ...
        if self.active_action is None:
            ...
            if dur < MIN_DUR:
                msg = f"[{t:.2f}s] ⚠ {dur:.2f}s 구간 너무 짧아 무시"
    ...
    def fill_gaps(self, total_sec: float, gap_min: float = 0.15) -> list:
        """구간 사이 갭을 action 7로 채운 최종 리스트 반환"""
...
def draw_ui(frame, cur_sec, total_sec, labeler: Labeler, paused: bool) -> np.ndarray:
    ...
    # 시간
    def fmt(s): m=int(s)//60; return f"{m:02d}:{s-m*60:05.2f}"
...
def save_csv(segments: list, path: str):
    ...
    cnt_by_action: dict = defaultdict(int)
    for s, e, a in segments:
        dur_by_action[a] += e - s
        cnt_by_action[a] += 1
...
def main():
    ...
    while True:
        ...
        elif key in (81, ord('a'), ord('A')):   # ← 또는 A
            cur_f = max(0, cur_f - int(fps * 5))

        elif key in (83, ord('d'), ord('D')):   # → 또는 D
